...
import argparse
import logging
from gitsuperlog import helper
from gitsuperlog.defaults import EMPTY_TREE_SHA

import git
from git.compat import (defenc, PY3)

from git.util import (Actor, Iterable, Stats, finalize_process)
...
parser.add_argument("-o", "--output", type=str, help="CSV output")
...
def __main__():
    ...
    for i in commits:
        ...
        for k in diff:
            ...
            insertions, deletions = helper.get_stats_from_patch(msg)

            netChange = insertions
            logging.debug("%s Insertions %s Deletions %s" %
                          (k.b_path, insertions, deletions))
            total += abs(netChange)
